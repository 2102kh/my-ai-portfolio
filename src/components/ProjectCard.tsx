...
const ProjectCard = ({ project }: { project: Project }) => {
  return (
    <div className="min-w-[300px] bg-white rounded-xl shadow-md overflow-hidden transition hover:shadow-xl">
      <div className="align-center bg-[var(--color-beige)] justify-center flex items-center ">
        {project.image && (
          <img src={project.image} 
               alt={project.title} 
               className="w-auto h-auto max-h-[220px] max-w-full object-contain rounded-xl" />
        )}
      </div>
      <div className="p-4">
        ...
        {project.link && (
          <Link href={project.link}>
            <span className="text-[var(--color-accent)] hover:underline">Google Play</span>
          </Link>
        )}
